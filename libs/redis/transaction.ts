...
export class CacheTransaction {
  ...
  */
  expire(key:string, ttl?:number): CacheTransaction {
  ...
  * @desc: 移除指定key的ttl.
  */
  persist(key:string):CacheTransaction {
  ...
  * @param value: 设置的值.
  */
  hset(key:string, field:string, value:any):CacheTransaction {
  ...
  * @desc: 获取hash表指定的数据.
  */
  hget(key:string, field:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  * @desc: 获取hash表的keys.
  */
  hkeys(key:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  * @desc: 清理hash表指定的数据.
  */
  hdel(key:string, ...fields:string[]):CacheTransaction {
  ...
  * @desc: 在集合中插入值.
  */
  sadd(key:string, value:string):CacheTransaction{
  ...
  * @desc: 在集合中移除指定成员.
  */
  sremove(key:string, ...values:string[]):CacheTransaction {
  ...
  * @desc: 返回集合中的元素个数.
  */
  scard(key:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  * @desc: 判读是否是集合中的元素.
  */
  sismember(key:string, value:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  * @desc: 返回集合中的所有成员.
  */
  smembers(key:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  */
  set(key:string, value:any, ttl?:number):CacheTransaction {
  ...
  * @desc: 获取指定的数据.
  */
  get(key:string, callback?:(err:Error,res:any)=>void):CacheTransaction {
  ...
  * @desc: 清理指定的数据.
  */
  del(key:string):CacheTransaction {
    this.redis.del(key);
    return this;
  };
